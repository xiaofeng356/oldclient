...
public class AutoCrystalRewrite extends Module {
    ...
    private final Timer _rotationResetTimer = new Timer();
    private final ConcurrentLinkedQueue<BlockPos> _placedCrystals = new ConcurrentLinkedQueue<>();
    private final ConcurrentHashMap<BlockPos, Float> _placedCrystalsDamage = new ConcurrentHashMap<>();
    private final ICamera camera = new Frustum();
    private double[] _rotations = null;
    private final ConcurrentHashMap<EntityEnderCrystal, Integer> _attackedEnderCrystals = new ConcurrentHashMap<>();
    ...
    @EventHandler
    private final Listener<EventClientTick> OnClientTick = new Listener<>(event ->
    {
        ...
        boolean isValidCrystal = crystal != null && mc.player.getDistance(crystal) < BreakRadius.getValue();
        ...
        if (isValidCrystal && (skipUpdateBlocks || _remainingTicks == Ticks.getValue())) // we are checking null here because we don't want to waste time not destroying crystals right away
        {
    ...
    @EventHandler
    private final Listener<EventNetworkPacketEvent> OnPacketEvent = new Listener<>(event ->
    {
        if (event.getPacket() instanceof SPacketSoundEffect) {
            ...
            if (packet.getCategory() == SoundCategory.BLOCKS && packet.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                ...
                new ArrayList<Entity>(mc.world.loadedEntityList).forEach(e ->
                {
                    ...
                    _placedCrystals.removeIf(pos -> pos.getDistance((int) packet.getX(), (int) packet.getY(), (int) packet.getZ()) <= 6.0);
                });
            }
        }
    });
        ...
        _placedCrystals.forEach(pos ->
        {
            ...
            if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + mc.getRenderManager().viewerPosX,
                ...
                if (_placedCrystalsDamage.containsKey(pos)) {
                    ...
                    final String damageText = String.valueOf(Math.floor(damage) == damage ? (int) damage : String.format("%.1f", damage));
                    GlStateManager.disableDepth();
