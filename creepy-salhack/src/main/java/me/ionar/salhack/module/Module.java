...
public class Module implements Listenable {
        ...
        for (Value v : this.getValueList()) {
            ...
            if (v.getValue() instanceof Enum) {
                ...
                msg.appendSibling(new TextComponentString(
                        valuePrefix + v.getName() + ChatFormatting.GRAY + " <" + options + ">"
        ...
        if (GuiScreen.isAltKeyDown() || GuiScreen.isCtrlKeyDown() || GuiScreen.isShiftKeyDown()) {
            if (key.contains(" + ")) {
                if (GuiScreen.isAltKeyDown() && key.contains("MENU")) {
                    String result = key.replace(Keyboard.isKeyDown(56) ? "LMENU + " : "RMENU + ", "");
                    return result.equals(keyCode);
                    ...
                    String result = key.replace(ctrlKey + " + ", "");
                    return result.equals(keyCode);
                } else if (GuiScreen.isShiftKeyDown() && key.contains("SHIFT")) {
                    String result = key.replace((Keyboard.isKeyDown(42) ? "LSHIFT" : "RSHIFT") + " + ", "");
                    return result.equals(keyCode);
                }
            }

            if (!ModuleManager.Get().IgnoreStrictKeybinds()) {
                if (keyCode.contains("SHIFT") || keyCode.contains("CONTROL") || keyCode.contains("MENU"))
    ...
    public void SignalValueChange(Value val) {
        SaveSettings();
    }
    ...
    public void SetClickGuiValueUpdate(boolean val) {
        m_NeedsClickGuiValueUpdate = val;
    }
    ...
    public String GetNextStringValue(final Value<String> val, boolean recursive) {
    ...
    protected void SendMessage(String message) {
        if (mc.player != null)
            SalHack.SendMessage(ChatFormatting.AQUA + "[" + GetArrayListDisplayName() + "]: " + ChatFormatting.RESET + message);
    }
