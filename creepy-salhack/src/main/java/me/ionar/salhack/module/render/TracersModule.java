    ...
    private final Listener<RenderEvent> OnRenderEvent = new Listener<>(event ->
    {
        ...
        for (Entity entity : mc.world.loadedEntityList) {
            if (shouldRenderTracer(entity)) {
                final Vec3d pos = MathUtil.interpolateEntity(entity, event.getPartialTicks()).subtract(mc.getRenderManager().renderPosX, mc.getRenderManager().renderPosY, mc.getRenderManager().renderPosZ);
