...
public class BlockInteractionHelper {
    ...
    public static ValidResult valid(BlockPos pos) {
        ...
        if (state.getBlock() == Blocks.AIR) {
            ...
            for (BlockPos pos1 : blocks) {
                IBlockState state2 = mc.world.getBlockState(pos1);

                if (state2.getBlock() == Blocks.AIR)
                    continue;

                for (final EnumFacing side : EnumFacing.values()) {
                    final BlockPos neighbor = pos1.offset(side);

                    boolean isWater = mc.world.getBlockState(neighbor).getBlock() == Blocks.WATER;

                    if (mc.world.getBlockState(neighbor).getBlock().canCollideCheck(mc.world.getBlockState(neighbor), false)
                            || (isWater && ModuleManager.Get().GetMod(LiquidInteractModule.class).isEnabled())) {
                        return ValidResult.Ok;
                    }
                }
            }
        ...
        /*
         * final BlockPos[] blocks = { pos.north(), pos.south(), pos.east(), pos.west(), pos.up() };
         *
         * for (BlockPos pos : blocks) { IBlockState state = mc.world.getBlockState(pos);
         *
    ...
    public static PlaceResult place(BlockPos pos, float distance, boolean rotate, boolean useSlabRule, boolean packetSwing) {
        ...
        boolean isSlabAtBlock = state.getBlock() instanceof BlockSlab;

        if (!replaceable && !isSlabAtBlock)
        ...
        if (!isSlabAtBlock) {
            ...
            if (result != ValidResult.Ok && !replaceable)
                return PlaceResult.CantPlace;
        }

        if (useSlabRule) {
            if (isSlabAtBlock && !state.isFullCube())
                return PlaceResult.CantPlace;
        }
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (mc.world.getBlockState(neighbor).getBlock().canCollideCheck(mc.world.getBlockState(neighbor), false)
                ...
                if (eyesPos.distanceTo(hitVec) <= distance) {
                    ...
                    EnumActionResult result2 = mc.playerController.processRightClickBlock(mc.player, mc.world, neighbor, side2, hitVec, EnumHand.MAIN_HAND);
    ...
    public static boolean IsLiquidOrAir(BlockPos pos) {
        ...
        return state.getBlock() == Blocks.WATER || state.getBlock() == Blocks.LAVA || state.getBlock() == Blocks.AIR;
    }
