...
public final class RenderUtil {
    ...
    public static float drawStringWithShadow(String name1, float x1, float y1, int color) {
        HudModule hud = (HudModule) ModuleManager.Get().GetMod(HudModule.class);

        if (hud != null && !hud.CustomFont.getValue())
            return (float) Wrapper.GetMC().fontRenderer.drawStringWithShadow(name1, x1, y1, color);

        return SalHack.GetFontManager().DrawStringWithShadow(name1, x1, y1, color);
    }

    public static float getStringWidth(String name1) {
        HudModule hud = (HudModule) ModuleManager.Get().GetMod(HudModule.class);

        if (hud != null && !hud.CustomFont.getValue())
            return Wrapper.GetMC().fontRenderer.getStringWidth(name1);

        return SalHack.GetFontManager().GetStringWidth(name1);
    }
    ...
    public static void drawSplitString(String name1, int x1, int y1, int k, int color) {
        /*
         * final HudModule hud = (HudModule) SalHack.INSTANCE.getModuleManager().find(HudModule.class);
         *
         * if (hud != null) { /* if (hud.CustomFont.getValue()) { GL11.glEnable(3042); GL11.glDisable(GL11.GL_DEPTH_TEST); GL11.glEnable(GL11.GL_POINT_SMOOTH);
         * GL11.glHint(GL11.GL_POINT_SMOOTH_HINT, GL11.GL_NICEST); GL11.glDepthMask(false); SalHack.INSTANCE.customFont.drawSplitString(name1, x1, y1, k, color);
         * GL11.glEnable(GL11.GL_DEPTH_TEST); GL11.glDisable(GL11.GL_POINT_SMOOTH); GL11.glDepthMask(true); } else
         */
        // Minecraft.getMinecraft().fontRenderer.drawSplitString(name1, x1, y1, k, color);
        // .color. }*/
        Minecraft.getMinecraft().fontRenderer.drawSplitString(name1, x1, y1, k, color);
    }
    ...
    public static void drawRect(int x, int y, int x1, int y1, int color, int customAlpha) {
        setupOverlayRendering();
        disableDefaults();
        GL11.glColor4d(getRedFromHex(color), getGreenFromHex(color), getBlueFromHex(color), customAlpha > 0 ? customAlpha : getAlphaFromHex(color));
    ...
    public static void drawRoundedRect(int x, int y, int x1, int y1, int radius, int color, int customAlpha) {
        disableDefaults();
        ...
        // Draw curves
        drawQuarterCircle((int) newX, (int) newY, radius, 0, color, customAlpha);
        drawQuarterCircle((int) newX1, (int) newY, radius, 1, color, customAlpha);
        drawQuarterCircle((int) newX, (int) newY1, radius, 2, color, customAlpha);
        drawQuarterCircle((int) newX1, (int) newY1, radius, 3, color, customAlpha);
        enableDefaults();
    }
    ...
    // 2 = Bottom Left
    // 3 = Bottom Right
    public static void drawQuarterCircle(int x, int y, int radius, int mode, int color, int customAlpha) {
        disableDefaults();
        GL11.glColor4d(getRedFromHex(color), getGreenFromHex(color), getBlueFromHex(color), customAlpha > 0 ? customAlpha : getAlphaFromHex(color));
        GL11.glBegin(GL11.GL_POLYGON);
        GL11.glVertex2d(x, y);
